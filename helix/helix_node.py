...
class HelixNode(GossipNode):
    """A very small reference implementation of a Helix node."""
    ...
    def load_state(self) -> None:
        """Load events from ``events_dir`` and ignore invalid ones."""
    ...
    def create_event(self, statement: str, *, private_key: str | None = None) -> Dict[str, Any]:
        """Create a new event for ``statement`` using ``microblock_size``."""
        ...
        if private_key is not None and event.get("originator_pub"):
    ...
    def import_event(self, event: Dict[str, Any]) -> None:
        """Validate ``event`` and store it locally."""
    ...
    def mine_event(self, event: Dict[str, Any]) -> None:
        """Attempt to mine all microblocks for ``event``."""
        ...
        for idx, block in enumerate(event["microblocks"]):
            ...
            if seed is not None and verify_seed(seed, block):
                chain = [seed]
            else:
    ...
    def finalize_event(self, event: Dict[str, Any]) -> None:
        """Process bets and rewards once ``event`` is closed."""

        if not event.get("is_closed"):
            return

        # Simple bet settlement: YES always wins for this demo
    ...
    def _handle_message(self, message: Dict[str, Any]) -> None:
        ...
        if msg_type in {GossipMessageType.NEW_EVENT, GossipMessageType.NEW_STATEMENT}:
            ...
            if (
                ...
                or seed_hex is None
                or idx is None
                or idx >= len(self.events[evt_id]["microblocks"])
            ):
                return
            ...
            for _ in range(1, depth):
                current = minihelix.G(current, len(seed))
                chain.append(current)

            try:
                event_manager.accept_mined_seed(self.events[evt_id], idx, chain)
            except Exception:
                return

            if self.events[evt_id].get("is_closed"):
    ...
    # ------------------------------------------------------------------
    def run(self) -> None:  # pragma: no cover - simple CLI helper
