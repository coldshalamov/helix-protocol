...
def test_accept_mined_seed_shorter_replacement():
    event = event_manager.create_event("abc", microblock_size=3)
    enc_long = bytes([2, 4]) + b"long" + b"inter"
    ...
    hdr = event["seeds"][0][0]
    l = event["seeds"][0][1]
    assert event["seeds"][0][2 : 2 + l] == b"a"
...
def test_accept_mined_seed_conditions():
    ...
    refund = event_manager.accept_mined_seed(event, 0, bytes([2, 2]) + b"bb")
    assert refund == 0
    hdr = event["seeds"][0][0]
    l = event["seeds"][0][1]
    assert event["seeds"][0][2 : 2 + l] == b"a"
    # higher depth should not replace
    refund = event_manager.accept_mined_seed(event, 0, bytes([3, 1]) + b"c")
    assert refund == 0
    assert event["seed_depths"][0] == 2
    # same depth should not replace
    refund = event_manager.accept_mined_seed(event, 0, bytes([2, 1]) + b"d")
    assert refund == 0
    hdr = event["seeds"][0][0]
    l = event["seeds"][0][1]
    assert event["seeds"][0][2 : 2 + l] == b"a"
